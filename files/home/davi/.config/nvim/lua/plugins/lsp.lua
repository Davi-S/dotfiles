...
return {
    ...
    config = function()
        ...
        for _, server_name in ipairs(servers_list) do
            -- Neovim automatically loads the config from 'nvim/lsp/[server_name].lua', so there is no need to call `vim.lsp.config()`
            -- The configs under 'nvim/lsp/[server_name].lua' were downloaded from the lspconfig and eddited as I please.
            -- I prefer to have the settings locally instead of a plugin dependency.
